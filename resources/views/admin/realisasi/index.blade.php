...
@section('page-title', 'Data Realisasi Bansos')

@section('topbar-actions')
    <a href="{{ route('admin.realisasi.create') }}"
...
<div class="bg-white border border-gray-200 rounded-lg p-4 mb-4">
    <form method="GET" action="{{ route('admin.realisasi.index') }}" class="flex flex-wrap gap-3 items-end">
        ...
        <table class="w-full text-sm min-w-[920px]">
            <thead>
                <tr class="border-b border-gray-100">
                    ...
                    <th class="text-right px-4 py-3 text-xs font-medium text-gray-500">KPM</th>
                    <th class="text-right px-4 py-3 text-xs font-medium text-gray-500">Nominal (Rp)</th>
                    <th class="text-left px-4 py-3 text-xs font-medium text-gray-500">Tgl Realisasi</th>
                    <th class="text-left px-4 py-3 text-xs font-medium text-gray-500">Sumber</th>
                ...
                @forelse($data as $row)
                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">
                            {{ $row->tanggal_realisasi?->format('d/m/Y') ?? '-' }}
                        </td>
                        <td class="px-4 py-3 text-xs text-gray-400 max-w-[100px] sm:max-w-[120px] truncate">
                            {{ $row->sumber_data ?? '-' }}
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-3 whitespace-nowrap">
                                <a href="{{ route('admin.realisasi.edit', $row) }}"
                                   class="text-xs text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('admin.realisasi.destroy', $row) }}" method="POST"
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-12 text-center text-sm text-gray-400">
                            Belum ada data realisasi. <a href="{{ route('admin.realisasi.create') }}"
